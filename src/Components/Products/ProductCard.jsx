  ...
  return (
    <div onMouseEnter={() => setShowImg(false)} onMouseLeave={() => setShowImg(true)}
      ...
      <div className="flex flex-col mb-1">
        ...
        <Link to={`/products/${product.SL}`} className="font-semibold my-1 leading-none hover:text-emerald-500 transition-colors md:text-lg text-sm">{product.title}</Link>
      </div>
      ...
      <div className="flex justify-between items-center mt-3">
        ...
        <button onClick={() => notify()} className="bg-emerald-100 hover:bg-emerald-500 text-emerald-600 hover:text-white font-semibold px-2 lg:px-6 md:py-2 py-1 rounded-md flex justify-between items-center gap-1 hover:-translate-y-1 active:scale-95 transition duration-300 md:text-sm text-xs cursor-pointer">
          <BsCart3 />
          <h1 className="">Add</h1>
        </button>
      </div>
